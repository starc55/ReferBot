import { getPrismaClient } from "@telegram-referral/database";
    ...
    repository,
    {
      async getChatMember(channelId, telegramUserId) {
        ...
        if (!Number.isSafeInteger(numericUserId)) {
          throw new Error("Telegram user ID exceeds JavaScript safe integer range");
        }
